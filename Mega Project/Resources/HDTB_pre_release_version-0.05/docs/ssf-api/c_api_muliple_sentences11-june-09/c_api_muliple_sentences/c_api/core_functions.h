			...
			case ' ':
				...
				if ( str1[0]!='\0' && str2[0]!='\0')
				{
					...
					else
					{
						...
						char attr[9][512] ; // To increase size of fs values to handle the 
								    // unicode string in feature structure -Rashid 11-junr-09
...
char * make_or_node_to_string ( or_node *OR)
{
	...
	str[0]='\0';				    // from 1000(org) to 2000 for current handling--Rashid Ahmad 11-june-09  
	for (i=0 ; i< OR->fs_count; i++)
	{
		strcat ( str, make_fs_struct_to_string ( OR->fs[i] ) );
		if ( i < OR->fs_count-1 )
			strcat (str, "|"); // replace pipe from space for morph feature structure -- Rashid Ahmad 11-june-09
	}
	return str;
}
...
char *make_fs_struct_to_string ( fs_struct *fs)
{
	...
	{
		int i;
		char *str=(char *)malloc(100*sizeof(char)); // change the size of an or node to handle the unicode string 
							    // in or node from 1000(org) to 2000 for current 
							    // handling -- Rashid Ahmad 11-june-09
		strcpy(str,"<fs ");
		GList *start=fs->keys;
		char *tmp;
