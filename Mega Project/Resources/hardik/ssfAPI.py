...
class Document():
    ...
    def __init__(self, fileName):
        print("Entering the init function in Document class with filename : ", fileName)
        self.header = None
        self.footer = None
        self.text = None
        self.nodeList = []
        ...
        self.upper = None
        print("Leaving the init function in Document class")

    def analyzeDocument(self):

        print("Entering the analysze Document fucntion in Document class with self : ", self)
        inputFD = codecs.open(self.fileName, 'r', encoding='utf8')
        sentenceList = findSentences(inputFD)
        print("Printing the senctence list..")
        temp_output_sentencelist=open("temp_output_sentencelist.txt","w")
        for i in sentenceList:
            print("sentence# :",i)
            temp_output_sentencelist.write(i)
...
def findSentences(inpFD):
    # print("Entering find Sentences with inpFD = ", inpFD)
    ...
    # print(*temp_list)
    # temp_list=(re.findall(sentenceRE, text, re.DOTALL))
    # for i in temp_list:
    #     print(i)
    return temp_list
...
if __name__ == '__main__':
    ...
    inputPath = sys.argv[1] #getting the input path as an arguement
    fileList = folderWalk(inputPath) #will append all the files in a list inside that folder mentioned above
    newFileList = []
    for fileName in fileList: #code to just filter out the files
    ...
    for fileName in newFileList:
        print("Printing the file names : ",fileName)
        d = Document(fileName)
